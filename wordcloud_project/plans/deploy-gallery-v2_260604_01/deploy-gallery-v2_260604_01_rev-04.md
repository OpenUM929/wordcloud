# 저장 갤러리 V2 — 상세보기 개선 및 매트릭스 자동 저장 기획서 (rev-04)
...
- **개정 이력**:
  ...
  - rev-04: `.chip` / `.chip--active` / `.chip--disabled` CSS 추가 (프로젝트 내 미존재 확인)
...
| 구분 | rev-03 | rev-04 |
|------|--------|--------|
| `.chip` CSS | 미정의 — 프로젝트 전체에 없음 | 6.5절에 CSS 블록 추가, Phase 2 체크리스트에 명시 |
...
> ⚠️ **rev-04 추가**: 아래 CSS 클래스는 프로젝트 전체에 미존재. 구현 시 `deploy_gallery.html` `<style>` 블록에 반드시 추가.
...
1. **`web/templates/deploy_gallery.html`**
   - `<style>` 블록에 `.source-badge`, `.chip`, `.chip--active`, `.chip--disabled` CSS 추가 *(rev-04 추가)*
...
| T-16 | 칩 스타일 적용 | 칩 클릭 후 시각 확인 | 선택 칩 파란 배경, 비활성 칩 흐리게 표시 |
...
| `web/templates/deploy_gallery.html` | 신규 CSS, 카드 배지, 모달 HTML/JS 전면 개선 |
...
- [ ] `.chip` / `.chip--active` / `.chip--disabled` CSS 클래스가 `<style>` 블록에 추가된다.
...
**계획서 저장 위치**: `D:\dev\wordcloud\wordcloud_project\plans\deploy-gallery-v2_260604_01\deploy-gallery-v2_260604_01_rev-04.md`
