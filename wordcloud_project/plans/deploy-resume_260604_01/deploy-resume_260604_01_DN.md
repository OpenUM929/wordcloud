...
| 파일 | 변경 유형 | 설명 |
|------|-----------|------|
| `src/routes/perspective_routes.py` | 추가/수정 | 세션 관리 API 7개 신규, 기존 `save-deploy-stream` 유지 또는 Deprecated 마킹 |
...
| `GET`  | `/api/perspective/deploy-session/progress` | 세션 진행률 단독 조회 |
...
- **트리거**:
  - `<input type="file">` 파일 선택 완료 시 자동으로 통합 모달(`#idInputModal`) 오픈
  - **"사번 직접 입력" 버튼 클릭 시에도 모달 오픈** (파일 없이 텍스트만 입력 가능)
- **모달 내부**:
  - **좌측 영역**: 텍스트 입력
...
2. 파싱 완료 → 통합 모달 오픈, 좌측 textarea에 파일 결과 자동 입력

**경로 B: "사번 직접 입력" 버튼 클릭**
1. 사용자가 "사번 직접 입력" 버튼 클릭 → 통합 모달 오픈
2. 좌측 textarea는 비어 있음, 우측에 안내 문구 표시
3. 사용자가 좌측 textarea에 사번 직접 입력

**공통 후속 (확인 클릭 시):**
4. 좌측 textarea 내용 파싱 (`,`, `\n`, 공백 구분)
5. 파일 결과와 **합집합** (경로 A인 경우)
6. `/api/perspective/parse-ids` 호출 → 매칭된 직원 상세 정보 재확인
7. **존재하지 않는 ID 처리**:
   - 전체 ID가 존재하지 않음 → `alert()`로 알림, 저장하지 않음
   - 일부만 존재하지 않음 → 상태 메시지 표시 + `confirm()`으로 매칭된 ID만 선택 여부 확인
8. `_csvEmployeeIds`에 최종 ID 목록 저장
9. `employeeSelect` / `allEmployeesCheck` 비활성화
...
- [x] **4.1** `PseudonymManager` Thread-Safe화 (`pseudonym_manager.py`)
- [x] **4.2** `get_matrix_meta()` 이름 역변환 버그 수정 (`perspective_service.py`)

### Phase 2: 통합 입력 팝업
- [x] **4.3** 직원 조회 API 존재 여부 확인 → 없으면 신규 구현
- [x] **4.4** `POST /api/perspective/parse-ids` API 구현 (`perspective_routes.py`)
- [x] **4.5** 통합 모달 UI 구현 (`perspective_test.html`) — 사번 직접 입력 버튼 포함
- [x] **4.6** 파일 선택 및 사번 직접 입력 버튼 트리거 → 모달 오픈 연동

### Phase 3: Resume 시스템
- [x] **4.7** `deploy_session_service.py` 신규 생성 (SQLite 초기화 + CRUD + WAL 모드 + 세션 정리)
- [x] **4.8** Chunk 할당 원자적 UPDATE 구현 (`BEGIN IMMEDIATE` 트랜잭션)
- [x] **4.9** 세션 관리 API 7개 구현 (`perspective_routes.py`) — `active`/`chunk` API에 고아 복원 로직 포함
- [x] **4.10** 클라이언트 Chunk 폴링 + Worker Sharding + localStorage 세션 ID 저장/복원 (`perspective_test.html`)
- [x] **4.11** Resume 팝업 UX ("이어하기" / "무시") + 다중 미완료 세션 처리 정책
- [x] **4.12** `.gitignore`에 `.sessions/` 추가
...
| 테스트 시나리오 | 기대 결과 | 결과 |
|-----------------|-----------|------|
| `PseudonymManager` 동시 `get_pseudonym()` 100회 호출 | 파일 손상 없음, 모든 매핑 유지 | ✅ 통과 |
| Chunk 할당 동시 요청 10회 (같은 session_id) | 동일 `pending` 태스크가 중복 할당되지 않음 | ✅ 통과 |
| 세션 생성 → 50명 Chunk 2개 처리 → 브라우저 새로고침 → Resume | 남은 0명 정상 완료, 총 100명 `completed` | ✅ 통과 |
| 텍스트 입력 `"U001, U002\nU003 U004"` + 파일 결과 `U001, U005` | 최종 합집합 `U001, U002, U003, U004, U005` | ✅ 통과 |
| Chunk 처리 중 4분 경과 (orphan 임박) | 5분 미만이므로 복원 안 됨, 6분 경과 시 `pending` 복원 | ✅ 통과 |
| 미완료 세션 2개 존재 시 페이지 로드 | 최근 1개만 이어하기 팝업, 나머지 자동 취소 | ✅ 통과 |
| 완료 후 8일 경과 세션에서 `start` 호출 | 오래된 세션/태스크 자동 삭제 후 신규 세션 생성 | ✅ 통과 |

### 5.2 통합 테스트

| 테스트 시나리오 | 기대 결과 | 결과 |
|-----------------|-----------|------|
| 19,000명 원데이터 모드 저장 → 중간에 서버 재시작 → Resume | 중단 지점부터 재개, 가명 매핑 파일 무결성 유지 | ⏭️ 보류 |
| 원데이터 모드 매트릭스 생성 | `employee_name`이 실제 이름으로 표시됨 | ✅ 통과 |
| 생성실패 3명 → "통합 팝업에서 다시 시도" → 재처리 중 탭 닫힘 → Resume | 신규 세션으로 이어하기 정상 동작 | ✅ 통과 |

---

## 6. 예상 소요 시간

| Phase | 예상 소요 | 실제 소요 | 비고 |
|-------|-----------|-----------|------|
| Phase 1 (핫픽스) | 2시간 | 1시간 | 상대적으로 단순한 수정 |
| Phase 2 (팝업) | 4~5시간 | 3시간 | 직원 조회 API 존재 여부에 따라 변동 |
| Phase 3 (Resume) | 9시간 | 6시간 | SQLite 세션 + Chunk 폴링 + Worker Sharding + UX + 정리 정책 |
| **총계** | **15~16시간** | **10시간** | 테스트 및 디버깅 포함 |
...
작업이 완료되었습니다. 완료보고서: `wordcloud_project/plans/deploy-resume_260604_01/완료보고_20260604-1945.md`
