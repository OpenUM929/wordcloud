...
| 2026-06-30 | §6 추가 | 욕설 리스트 동음이의어 '년'(年/근속) 오탐 보고 → 단독 '년' 제거(복합 욕설 유지). 회귀 pf-13~16 추가 |
...
## 6. 추가 — 욕설 리스트 동음이의어 '년'(年/근속) 오탐 (2026-06-30)

- **관찰**: `입사 3년차`/`2025년`/`5년 근속` 등 연도·근속 표현의 `년`이 욕설로 오집계됨.
- **원인**: 기본 `profanity_words`에 **단독 `년`**(성적 비하어)이 포함. Kiwi가 연도 `년`(年, 의존명사)과 비하어 `년`을 **둘 다 NNB로 동일 태깅** → `_detect_by_morpheme`(L156) 정확일치에 둘 다 걸림. 품사로 구분 불가한 진성 동음이의어(사정/가슴/젖과 동류). 단음절이라 gap 패턴(`n<2` skip)은 무관 — 형태소 정확일치 경로 단독 원인.
- **결정/수정(사용자 승인)**: `_load_config` 기본 리스트에서 **단독 `년` 제거**. 욕설 복합어는 자체 항목으로 검출 유지 — 실측: `미친년`→`[미친년/NNG]`, `쌍년`→`[쌍년/NNG]`, `개년`→`[개년/NNB]`(리스트 항목 일치), `지랄년`→`지랄`(리스트 항목)로 검출. 따라서 비하어 탐지력 실질 손실 없음, HR 코퍼스에서 단독 `년` 비하어는 사실상 부재·연도 표현 압도적.
- **검증**: 회귀 `profanity_cases.json`에 `pf-13~16` 추가 — 연도/근속 3건 검출 0, `미친년` 정탐 유지. 전체 16건 중 15 PASS, 1(`pf-06`)은 기존 HEAD 동일 실패(본 건 무관). 욕설 2계층 로직·시그니처 불변.
