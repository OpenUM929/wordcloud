# -*- coding: utf-8 -*-
"""입력 칸(장점/단점) 라벨이 실제 문장 내용과 얼마나 어긋나는가 — 원데이터 전수 센서스 — 2025년분.

대상: `plans/_datasets/kote_finetune/emotion/weak_export_260623.jsonl`
...
SRC = DS + "emotion/weak_export_260623.jsonl"
...
BATCH_FIELD = {           # RUNBOOK 2026-06-23행 + 약점부재 시그니처 5/5 일치로 확정
    "batch_20260622_0": "장점",   # 장점_3차 (약점부재 1.31%)
    "batch_20260623_2": "장점",   # 장점_2차 (1.45%)
    "batch_20260622_2": "단점",   # 단점_3차 (39.59%)
    "batch_20260623_1": "단점",   # 단점_1차 (31.78%)
    "batch_20260623_3": "단점",   # 단점_2차 (33.21%)
}


def field_of(rid):
    """2025년 자료: 배치 id -> 입력칸. (연도 확정 = 25.csv 문장 포함율 99.5~99.9%)"""
    return BATCH_FIELD.get(rid.split("-")[0])
...
json.dump(res, open(OUT + "field_census_2025_260728.json", "w", encoding="utf-8"),
          ensure_ascii=False, indent=2)
print("저장:", OUT + "field_census_2025_260728.json", f"| 총 {time.perf_counter() - t0:.1f}초")
