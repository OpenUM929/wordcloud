...
print(f"\n[2] 추출 — n={a.n} seed={a.seed} weight={a.weight} stratify={a.stratify}")
rnd = random.Random(a.seed)

# 추출틀(sampling frame) — --weight 가 "무엇을 1단위로 세는가"를 정한다.
#   occurrence : 원행 그대로. 동일 문장이 출현 횟수만큼 들어가 운영 구성을 재현하나,
#                동일 문장은 판정이 결정론적으로 같아 급내상관 ρ=1 → 설계효과 deff>1 이 붙는다.
#   unique     : 고유 (칸,문장) 1개씩. 중복이 없어 deff=1 이며 SRS 분산식을 그대로 쓸 수 있다.
#
# 🔴 2026-08-04 수식 감사 수정 — 구판은 `if a.stratify:` 가 `--weight` 분기보다 먼저 걸려
#    `--stratify --weight unique` 를 주면 **unique 가 조용히 무시되고 occurrence 로 뽑혔다.**
#    추출틀을 먼저 정하고 층화를 그 위에 얹도록 바꿔 두 옵션이 조합되게 했다.
#    occurrence 경로의 결과는 구판과 완전히 동일하다(동일 시드 → 동일 표본).
if a.weight == "unique":
    first = {}
    for rid, f, t in rows:
        first.setdefault((f, t), rid)
    units = [(first[k], k[0], k[1]) for k in sorted(uniq)]
else:
    units = rows
print(f"    추출틀 단위 = {len(units):,} ({a.weight})")

if a.stratify:
    # 칸 비례배분 층화 — 층별 표본수를 추출틀 비율로 고정하고, 잔여는 큰 소수부 순으로 배분
...
# 구성 검정 — 표본 칸 구성이 **추출틀**과 일치하는가(z)
# (occurrence 면 추출틀 = 모집단 원행이므로 구판과 동일한 검정이다)
pop = collections.Counter(f for _, f, _ in units)
