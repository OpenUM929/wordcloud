...
BATCH = os.environ.get("CENSUS_BATCH", "batch_20260714_0")
YEAR = int(os.environ.get("CENSUS_YEAR", "2025"))
...
print(f"{YEAR} 배치 원본 {n_raw:,}행 | 절 제외 {n_clause:,} | 문장 {n_sent:,} | 고유 {len(occ):,}")
...
res = dict(
    source=SRC, batch=BATCH, year=YEAR, model=MODEL, device=dev,
...
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), f"yearcensus_{YEAR}_260727.json")
