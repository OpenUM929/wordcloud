...
|------|-----------|-----------|
| 2026-08-13 | 전체 | 최초 작성 |
| 2026-08-20 | §3, §5 | 착수 시 실측 — §3 핵심 변경(`<nav>`→`<div>`)은 **이미 커밋 `af6969d`("배치 병합·불용어 일괄등록 백엔드 + 페이징 버그 수정")에 반영되어 있었음**(이 계획서 상태는 갱신되지 않고 Todo로 남아있던 것, `git log -p`로 확인). 미반영 상태였던 선택 항목(§3 사문 CSS 정리)만 이번에 적용 — `settings_hub.html:88~113` `.pagination-container button` 계열 4규칙(실제 마크업 `<a class="page-link">`와 불일치하는 사문, F5) 삭제, `.pagination-container`(81~86행) 컨테이너 규칙은 유지 |
...
| T6 | 사문 CSS 삭제 후 회귀 | 삭제한 4규칙이 다른 화면에서 참조되는지 전수 grep(`.pagination-container button`) | 참조 0건 | ✅ 통과 — `settings_hub.html` 1파일 내에서만 정의·사용, 다른 템플릿 참조 없음 |
