...
|------|-----------|-----------|
| 2026-08-13 | 전체 | 최초 작성 |
| 2026-08-13 | D-3, §4.3, §4.4 | 사용자 확정: 업로드 버튼을 **「이름 업로드」/「일반 업로드」 2개로 분리**. 이름 업로드는 항상 `인명` 카테고리로 `stopwords_names.json`(배포 제외)에 저장, 일반 업로드는 항상 기존 `stopwords.json`에 저장(카테고리 선택 가능, `인명` 제외). D-3 카테고리 드롭다운 방식은 폐기 |
...
| D-3 | 실명 저장 위치 + 업로드 UX | (A) `stopwords.json` 에 그대로 저장 / (B) `src/configs/stopwords_names.json` 으로 분리 + 배포 제외 목록에 추가 | **B 채택 — 사용자 확정(2026-08-13).** 실측상 `src/configs/` 는 배포 패키지에 포함된다(`build_deploy.ps1:34-41` 의 `$ExcludeDirs`·`$ExcludeFiles` 에 없음). 불용어 목적상 **이름은 가명이 아닌 실명이어야 하므로**(원문에서 지워야 함) 가명화로 회피할 수 없다 → 파일을 분리하고 `$ExcludeFiles` 에 `stopwords_names.json` 을 추가하는 것이 유일한 차단책. **추가로 사용자가 업로드 UX 자체를 분리하도록 지정**: 카테고리 드롭다운 1개로 통합하지 않고, 화면에 **「이름 업로드」**(항상 `인명` 카테고리 → `stopwords_names.json`)와 **「일반 업로드」**(기존 카테고리 선택 → 기존 `stopwords.json`, `인명` 카테고리는 선택지에서 제외) 버튼을 각각 둔다. 운영 서버에서는 이름을 업로드로 다시 넣는다 |
