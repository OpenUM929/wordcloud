# -*- coding: utf-8 -*-
"""능동학습 5라운드 큐 — bare NP **양 필드 균형** 확장(c4 성공 노선 계속).

배경: c4(bareNP 양필드 페어, gold3+silver230)가 c3_neu149를 57.7→73.2%·부recall 0.5→0.688로
      끌어올리고 8c_hard·baseline 긍↔부 0을 지켰다(IMPROVEMENT_HISTORY 4차 DN). 방향이 옳으므로
      c5는 동일 방식으로 **다음 트랜치**를 캔다. 잔여 헤드룸: c3_neu149 부recall 0.688(부→긍 2 잔존)
      + 신규 긍→부 1(감시). 이번 큐는 c4에서 쓴 233행(gold3+silver230)을 제외한 나머지 bare NP 풀.

r4와 동일한 필터·버킷·균형 로직. 차이는 LEAK_FILES에 c4 gold/silver 추가 + 산출 파일명뿐.

필터:
  - bare NP 휴리스틱: 종결서술어 없이 명사로 끝나는 단편(다/음/함/됨/임/기/요/… 종결·ㅁ명사화 제외).
  - field 有(단점/장점)만. model_out 추론으로 flip-risk 우선순위.

버킷:
  - 단점 bare NP (목표 부정): 모델이 부정이 아닌 것 우선 = 가르칠 지점.
  - 장점 bare NP (목표 긍정): 모델이 긍정이 아닌 것 우선.
  - 두 버킷 동수(min)로 잘라 균형(단방향 투입 시 반대 필드 긍↔부 누수).

누수 가드: 전 학습셋(+c4 gold/silver) + 전 테스트셋(baseline·gold_8c_test·c3_neu149) 텍스트 제외.
산출: eval/review/bareNP_r5_260707.jsonl (게시판 스키마, human_decision=null, ai_reference=모델힌트).
"""
import argparse
import glob
import json
...
# 전 학습셋(+c4) + 전 테스트셋(finetune_sentiment.TRAIN_FILES/TEST_SETS와 일치) — 누수/중복 제외
LEAK_FILES = (
    'gold_8c_train_260706.jsonl', 'gold_active_260707.jsonl',
    'gold_active_260707_c2.jsonl', 'silver_active_260707_c2.jsonl',
    'gold_active_260707_c4.jsonl', 'silver_active_260707_c4.jsonl',   # ← c5 신규 제외(c4 사용분)
...
def main():
    ...
    ap.add_argument('--model', default=os.path.join(DATASET_DIR, 'model_out'))  # c4 재학습 산출
    ...
    out = os.path.join(REVIEW, 'bareNP_r5_260707.jsonl')
    with open(out, 'w', encoding='utf-8') as w:
        for r in queue:
            w.write(json.dumps({
                ...
                'source_file': r.get('source_file'), 'note': 'bareNP_r5',
            }, ensure_ascii=False) + '\n')
